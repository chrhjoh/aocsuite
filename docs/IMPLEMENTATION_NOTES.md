...
- **I-10 Resolved: runtime-root resolution rejects unsafe values without panicking.** Storage-owned `get_aocsuite_dir` returns typed `LayoutError` values, treats `AOCSUITE_DATA_DIR` as a direct root, falls back through `XDG_DATA_HOME` and `HOME`, and preserves valid non-Unicode paths. `RuntimeLayout::new` separately validates explicit roots. Deterministic private-seam tests cover precedence, empty, relative, absolute, non-Unicode, and missing environment values (`aocsuite-storage/src/layout.rs`).
...
- **I-13 Resolved: config and session failures return typed errors.** Explicit-path configuration reads/writes and CLI prompts propagate `AocConfigError`; invalid HTTP-header characters in sessions return `AocClientError::Session` rather than panicking. Regression coverage verifies non-mutating reads, precedence, failed writes, malformed configuration preservation, session permissions, and invalid/valid session header construction (`aocsuite-config/src/lib.rs`, `aocsuite-client/src/lib.rs`).
