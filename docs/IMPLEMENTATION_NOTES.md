...
- **I-20 Partially resolved: active solutions have typed puzzle targets.** `SolverFile::ActiveSolution` now stores a day/year directly, preventing nested and nonsolution active-link targets. `AocContentFile` can still panic for missing days, and active-link replacement can still remove a regular file or lose the previous link on failure (`aocsuite-fs/src/file.rs:21-26`, `184-198`, `aocsuite-lang/src/utils.rs:81-96`). Test invalid constructors/targets and failed link replacement while preserving the previous file.
