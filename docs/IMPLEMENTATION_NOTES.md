...
- **I-02 Resolved in the working tree: a requested day/year selects its solution.** `compile` and `run` now activate the requested Rust/Python day before execution (`aocsuite-lang/src/lib.rs:19-47`, `55-94`). Keep regression coverage for distinct day-A/day-B solutions, fresh runs, and a run after template editing.
- **I-03 Solver results can be stale.** Every run uses `<runtime-root>/result.json`; the file is not cleared before execution and is removed only after successful parsing (`aocsuite-lang/src/lib.rs:18-32`, `aocsuite-lang/src/utils.rs:61-65`). A failed or interrupted run can leave output for a later run to consume. Test pre-seeded stale output, malformed output, and failure cleanup.
