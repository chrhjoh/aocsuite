...
- **I-10 Resolved: runtime-root resolution rejects unsafe values without panicking.** Public storage-owned `get_aocsuite_dir` returns typed `LayoutError` values, treats `AOCSUITE_DATA_DIR` as a direct root, falls back through `XDG_DATA_HOME` and `HOME`, and preserves valid non-Unicode paths. Callers pass the resolved path to `RuntimeLayout::new`; layout tests use explicit temporary roots (`aocsuite-storage/src/layout.rs`).
...
- **I-13 Resolved: config and session failures return typed errors.** Explicit-path configuration reads/writes and CLI prompts propagate `AocConfigError`; missing sessions remain optional while other read failures propagate, and invalid HTTP-header characters return `AocClientError::Session` rather than panicking. Regression coverage verifies non-mutating reads, transactional failed writes, lowercase persistence, malformed configuration preservation, session permissions, and invalid/valid session header construction (`aocsuite-config/src/lib.rs`, `aocsuite-cli/src/app.rs`, `aocsuite-client/src/lib.rs`).
...
- **I-15 Resolved: read-only language APIs avoid setup and subprocess failures propagate.** Explicitly constructed language runners avoid setup until execution, package mutation, or solver preparation requires it. Rust `cargo tree`/`clean`, Python `pip list`, browser launch, and interactive Git return typed failures for nonzero subprocess status. Regression tests cover shared command status handling and browser launch failure (`aocsuite-lang/src/utils.rs`, `aocsuite-editor/src/lib.rs`).
...
- **I-19 Resolved: `run --test` rejects missing example files.** The built-in example path is checked as a regular file before language setup, compilation, or solver execution; missing examples return `NotFound`, while `open` explicitly ensures an empty flat example file without overwriting existing content (`aocsuite-cli/src/app.rs`, `aocsuite-storage/src/layout.rs`). Regression tests cover missing, directory, existing, and idempotently ensured example paths.
...
- Validated puzzle/language values now live under `aocsuite_utils::domain`; configuration reads use explicit paths with privately captured environment values, and Clap/prompting remain in the CLI.
