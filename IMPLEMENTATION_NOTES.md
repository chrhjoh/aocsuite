...
Review snapshot: 2026-07-18. This is an issue and test inventory only; no fixes or tests have been implemented.
...
- Defer automatic runtime migration for existing installations until it is explicitly requested. Do not make migration a prerequisite for the initial TUI or bug-fix work.
...
- **I-02 A requested day/year does not select its solution.** Rust and Python compile/run ignore day and year and execute the global `solution.rs`/`solution.py` link last changed by `open` or `template` (`aocsuite-cli/src/app.rs:48-98`, `aocsuite-lang/src/rust/solver.rs:7-39`, `aocsuite-lang/src/python/solver.rs:10-34`). This can compute or submit an answer from the wrong solution. Test distinct day-A/day-B solutions for both languages, fresh runs, and a run after template editing.
- **I-03 Solver results can be stale or cross-contaminated.** Every run uses `<runtime-root>/result.json`; the file is not cleared before execution and is removed only after successful parsing (`aocsuite-lang/src/lib.rs:18-32`, `aocsuite-lang/src/utils.rs:61-65`). Concurrent or interrupted runs can read/delete each other's output or consume stale data. Test pre-seeded stale output, malformed output, cancellation/failure cleanup, and synchronized concurrent runs with unique answers.
- **I-04 Python cannot run from a fresh runtime.** Python execution requires `main.py`, but `setup_solver` is a no-op and normal CLI flows never request `SolveFile::Main` (`aocsuite-lang/src/python/solver.rs:28-57`, `aocsuite-lang/src/lib.rs:58-68`). Test a fresh Python open/run workflow before any generated files exist.
...
- **I-12 Relative custom input paths resolve from the language runtime.** `run --test FILE` passes a relative path to a child whose current directory is changed to the Rust/Python root (`aocsuite-cli/src/app.rs:55-61`, `aocsuite-lang/src/rust/solver.rs:34-39`, `aocsuite-lang/src/python/solver.rs:28-34`). Test invocation from a separate temporary working directory.
...
- **I-14 Cache metadata failures are ignored and writes are not atomic or locked.** Metadata writes discard errors; config, content, and metadata use direct read-modify-write operations (`aocsuite-fs/src/file.rs:98-102`, `168-181`, `201-217`, `aocsuite-config/src/lib.rs:34-44`, `86-89`). Test unwritable metadata, interrupted writes, and barrier-controlled concurrent writers; successful submission invalidation must be observable and reliable.
- **I-15 Query and cleanup APIs perform setup, while several process failures look successful.** Runner resolution always creates the root and runs solver/environment setup, including list and clean operations (`aocsuite-lang/src/lib.rs:78-83`, `96-105`, `140-163`). Package-list failures become empty lists; Rust cache clean, browser launch, and interactive Git ignore nonzero status (`aocsuite-lang/src/rust/dependencies.rs:47-76`, `aocsuite-lang/src/python/dependencies.rs:52-82`, `aocsuite-lang/src/rust/solver.rs:52-58`, `aocsuite-client/src/lib.rs:62-75`, `aocsuite-cli/src/git.rs:103-112`). Test command failures with fake executors and assert read-only/cleanup calls do not create environments.
...
- **I-20 Public path/link types allow panic or destructive invalid states.** `AocContentFile` fields are public, so puzzle/input with `day: None` later panics. Nested `LinkedSolution` targets can remove the active link and create a self-referential symlink; replacing a regular active file removes it first (`aocsuite-fs/src/file.rs:21-26`, `184-198`, `aocsuite-lang/src/lib.rs:47-75`, `aocsuite-lang/src/utils.rs:81-96`). Test invalid constructors/targets and failed link replacement while preserving the previous file.

### P2: Lower-Severity and Portability

- **I-21 Generated Python stubs print literal interpolation syntax.** Double braces make `{len(input)}` literal (`aocsuite-lang/src/python/solver.rs:59-69`). Test generated source with known input.
...
- **I-24 Generated runtime infrastructure has no migration/version boundary.** Existing `main.*` and generated Cargo files are never updated after first creation, so harness fixes do not reach existing users (`aocsuite-lang/src/lib.rs:58-68`, `aocsuite-lang/src/rust/solver.rs:44-49`, `aocsuite-lang/src/rust/dependencies.rs:8-22`). Automatic migration is deferred; retain this as a follow-up item and add upgrade fixtures only when migration work is explicitly scheduled.
...
- Use barriers/channels rather than sleeps for cache/config concurrency tests. Serialize solver runs in the TUI; give each queued job an ID for stale-completion handling.
...
- **`aocsuite-config`:** source precedence, typed parsing, non-interactive set/remove, secret handling, Unix permissions, malformed JSON, failed writes, and concurrent updates.
...
- **`aocsuite-fs`:** pure paths; cache miss/hit/offline/refresh; input persistence; parser/fetch failure preserving good data; metadata corruption; submission invalidation; clean day/year/all scope and idempotence; concurrent writers.
...
- **`aocsuite-lang`:** shared Rust/Python contract for setup, selected solution, compile/run, custom/example input, part selection, public result fields, unique result files, command failures, package list/add/remove/clean, library names, symlink safety, generated templates, migrations, and concurrency.
...
- `cargo test --workspace` passes but runs zero tests.
